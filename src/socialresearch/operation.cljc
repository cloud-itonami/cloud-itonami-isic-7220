(ns socialresearch.operation
  "OperationActor -- one research operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor (StudyOps-
  ...
  the governor is clean -- see `socialresearch.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [socialresearch.studyopsllm :as studyopsllm]
...
(defn build
  ...
  `socialresearch.store/Store`).
  opts:
    :advisor      -- a `socialresearch.studyopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (studyopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; StudyOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (studyopsllm/-advise advisor store request)]
            {:proposal p :audit [(studyopsllm/trace request p)]})))
