(ns socialresearch.sim
  "Demo driver -- `clojure -M:dev:run`. Walks a clean study through
  intake -> protocol verification -> data-reproducibility-risk
  screening -> human-subjects-review screening -> findings-report-
  publication proposal (always escalates) -> human approval -> commit,
  then shows five HARD holds (a jurisdiction with no spec-basis, an
  under-replicated study below its own recorded minimum-required
  replication count, an unresolved data-reproducibility risk screened
  directly via `:risk/screen`, an unconfirmed human-subjects-review
  screened directly via `:ethics/screen` [never via an actuation op
  against an unscreened study -- see this actor's own governor ns
  ...
  `cultural`'s, `reserve`'s, `proserv`'s, `sportsevent`'s,
  `recreation`'s, `sportsclub`'s, `partyops`'s, `memberorg`'s,
  `commrepair`'s and `applianceshop`'s ADR-0001s already recorded],
  and a double publication of an already-processed study) that never
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== study/intake study-1 (JPN, clean; replicated 5 >= minimum 3, human-subjects-review confirmed, no data-reproducibility risk) ==")
    ...
    (println "== ethics/screen study-1 (clean; escalates -- human approves) ==")
    (println (exec! actor "t3b" {:op :ethics/screen :subject "study-1"} operator))
    ...
    (println "== ethics/screen study-5 (involves human subjects, review unconfirmed -> HARD hold, never reaches a human) ==")
    (println (exec! actor "t9" {:op :ethics/screen :subject "study-5"} operator))

    (println "== actuation/publish-findings-report study-1 AGAIN (double-publication -> HARD hold) ==")
    (println (exec! actor "t10" {:op :actuation/publish-findings-report :subject "study-1"} operator))
